...
| Track user attributes/events/purchases | `braze_users_track` | Requires approval and `confirmed: true`; validate payload first. Prefer `braze_create_user` for simple "create user by email" requests. If this tool receives an email-only attributes object, the MCP server normalizes it into an alias-only user create. |
